...
class App extends Component {
  ...
  addVisitedCountry = id => {
    ...
    const visitedCountry = countriesList.map(each => {
      if (each.id === id) {
  ...
  removeVisitCountry = id => {
    const {countriesList} = this.state
    const addToCountries = countriesList.map(eachCountry => {
      if (eachCountry.id === id) {
        const updateList = {...eachCountry, isVisited: false}
        return updateList
      }
      return eachCountry
    })
    this.setState({countriesList: addToCountries})
  }
